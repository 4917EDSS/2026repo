...
// Velocity
const float sensorDistance = 0.025; // Meters
const int velThreshold = 150; // Threshold at which the ir sensors detect the ball for velocity calculation
...
void loop() {
  ...
  // Get velocity at the start to be declared by the time its used
  int16_t velocity = (int16_t) getVelocity(sensor0, sensor1);
  ...
  static int displayVelocity = 0;

  if(velocity != -1) {
    displayVelocity = velocity
  }
  printf(output, "ADC0: %d\nADC1: %d\nVel: %d", sensor0, sensor1, displayVelocity);
...
float getVelocity(int ir0, int ir1) {
  static bool velLock0 = false;
  static unsigned long velStartTime = 0;
  ...
  if(ir0 <= velThreshold) {
    ...
    if(ir1 <= velThreshold) {
      //velEndTime = millis();
      float returnData = sensorDistance / (millis() - velStartTime);
      return returnData;
