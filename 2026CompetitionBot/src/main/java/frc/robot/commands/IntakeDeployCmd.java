// Copyright (c) FIRST and other WPILib contributors.
// Open Source Software; you can modify and/or share it under the terms of
// the WPILib BSD license file in the root directory of this project.

package frc.robot.commands;

import edu.wpi.first.wpilibj2.command.Command;
import frc.robot.Constants;
import frc.robot.subsystems.HopperSub;
import frc.robot.subsystems.IntakeSub;

/*
 * You should consider using the more terse Command factories API instead
 * https://docs.wpilib.org/en/stable/docs/software/commandbased/organizing-command-based.html#defining-commands
 */
public class IntakeDeployCmd extends Command {
  private final IntakeSub m_intakeSub;
  private final HopperSub m_hopperSub;

  /** Creates a new IntakeDeployCmd. */
  public IntakeDeployCmd(HopperSub hopperSub, IntakeSub intakeSub) {
    m_intakeSub = intakeSub;
    m_hopperSub = hopperSub;

    // Use addRequirements() here to declare subsystem dependencies.
    addRequirements(intakeSub);
  }

  // Called when the command is initially scheduled.
  @Override
  public void initialize() {
    //deploy intake
    m_intakeSub.setTargetDeployAngle(Constants.Intake.kDeployOutAngleDeg);
    //start belt motors
    m_intakeSub.setBeltPower(Constants.Intake.kBeltPower);
  }

  // Called every time the scheduler runs while the command is scheduled.
  @Override
  public void execute() {}

  // Called once the command ends or is interrupted.
  @Override
  public void end(boolean interrupted) {
    m_intakeSub.setTargetDeployAngle(25);//(Constants.Intake.kDeployInAngleDeg);
    m_intakeSub.setBeltPower(0.0); // Turns off the belts once the intake is all the way in
  }

  // Returns true when the command should end.
  @Override
  public boolean isFinished() {
    if(m_hopperSub.isFull()) {
      return true;
    }
    return false;
  }
}
